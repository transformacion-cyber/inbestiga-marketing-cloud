/* ===== INBESTIGA v17.9.2 · COLLABORATIVE WORKSPACE · FLEXIBLE DELIVERY UX (v17.12.11) ===== */
(() => {
  "use strict";
  ...
  const DELIVERY_REQUIREMENT_LABELS = Object.freeze({
    none: "Sin evidencia obligatoria",
    any: "Cualquier tipo de evidencia",
    file: "Archivo o captura obligatoria",
    link: "Enlace obligatorio"
  });
  const DELIVERY_MARKERS = Object.freeze({
    none: "\u2063\u200B\u2063",
    any: "\u2063\u200C\u2063",
    file: "\u2063\u200D\u2063",
    link: "\u2063\u2060\u2063"
  });
  function deliveryRequirementFrom(value) {
    const source = str(value?.description ?? value);
    for (const [key, marker] of Object.entries(DELIVERY_MARKERS)) if (source.includes(marker)) return key;
    return "any";
  }
  function stripDeliveryRequirement(value) {
    let source = str(value);
    for (const marker of Object.values(DELIVERY_MARKERS)) source = source.split(marker).join("");
    return source.trim();
  }
  function withDeliveryRequirement(value, requirement = "any") {
    const clean = stripDeliveryRequirement(value);
    const key = DELIVERY_MARKERS[requirement] ? requirement : "any";
    return `${clean}${DELIVERY_MARKERS[key]}`;
  }
  function deliveryRequirementField(id, selected = "any") {
    return `<label class="v179-field full v171210-delivery-rule"><span>Requisito para entregar</span><select id="${escHtml(id)}">${Object.entries(DELIVERY_REQUIREMENT_LABELS).map(([key,label]) => `<option value="${key}" ${key===selected?"selected":""}>${escHtml(label)}</option>`).join("")}</select><small>El archivo es opcional salvo que el jefe elija expresamente “Archivo o captura obligatoria”.</small></label>`;
  }
  function deliveryRequirementDetailMarkup(task) {
    const key = deliveryRequirementFrom(task);
    const explanation = key === "none" ? "Puedes entregar solo con una explicación, o añadir enlaces y archivos si ayudan a revisar." : key === "any" ? "Sirve un comentario claro, un enlace, un archivo o una captura." : key === "file" ? "Debes adjuntar al menos un archivo o una captura de pantalla." : "Debes incluir al menos un enlace válido.";
    return `<section class="v179-detail-section v171210-requirement-detail"><div class="v179-section-head"><div><span>FORMA DE ENTREGA</span><h4>${escHtml(DELIVERY_REQUIREMENT_LABELS[key])}</h4></div><b>${key === "none" ? "Flexible" : "Requerida"}</b></div><p>${escHtml(explanation)}</p></section>`;
  }
  function responsibleReviewerName(taskId) {
    try {
      const operation = window.INBESTIGA_V1711?.operation?.(taskId);
      if (operation?.responsible_member_id) return memberNameById(operation.responsible_member_id);
    } catch (_) {}
    return "responsable final";
  }
  ...
  function ensureFormEnhancements() {
    ...
    if (taskForm && !document.getElementById("v171210TaskDeliveryRequirement")) {
      const descriptionField = document.getElementById("taskDescription")?.closest("label");
      descriptionField?.insertAdjacentHTML("afterend", deliveryRequirementField("v171210TaskDeliveryRequirement", "any"));
    }
    ...
    if (taskUpdate) {
      ...
      statusSelect?.querySelectorAll('option[value="en_revision"],option[value="corregido"]').forEach((option) => option.remove());
      taskUpdate.querySelectorAll("#v179UpdateFiles,#v179UpdateLinks").forEach((node) => node.closest(".v179-field,.v179-form-block")?.remove());
    }
  ...
  async function handleTaskCreate(form) {
    ...
    const taskId = await createLegacyEntity({
      ...
      params: {
        p_title: title,
        p_description: withDeliveryRequirement(document.getElementById("taskDescription")?.value?.trim() || "", document.getElementById("v171210TaskDeliveryRequirement")?.value || "any"),
  ...
  async function createSubmission(taskId, { comment, files, links }) {
    ...
    const task = taskById(taskId);
    const requirement = deliveryRequirementFrom(task);
    const cleanComment = str(comment).trim();
    validateFiles(files);
    if (requirement === "any" && !files.length && !links.length && !cleanComment) throw new Error("Agrega una explicación, un enlace, un archivo o una captura.");
    if (requirement === "file" && !files.length) throw new Error("Esta tarea requiere un archivo o una captura de pantalla.");
    if (requirement === "link" && !links.length) throw new Error("Esta tarea requiere al menos un enlace válido.");
    const { data, error } = await callRpc("ibm_v179_create_submission", { p_task_id: taskId, p_comment: cleanComment || "Tarea marcada como realizada y enviada a revisión", p_links: links.map((url) => ({ url })) });
    if (error) throw error;
  ...
  async function handleTaskUpdate(form) {
    ...
    if (["en_revision", "corregido"].includes(status)) throw new Error("Para enviar el resultado usa el botón “Entregar tarea”.");
    ...
    form.reset();
    notify("Progreso actualizado", "El avance quedó guardado. La entrega se realiza desde el botón principal de la tarea.", "success");
  }
  ...
  function deliveryModal(taskId) {
    ...
    if (["en_revision","corregido"].includes(status)) return notify("Entrega ya enviada", `Está esperando revisión de ${responsibleReviewerName(taskId)}.`, "warning");
    const requirement = deliveryRequirementFrom(task);
    const body = `<div class="v179-modal-grid v171210-delivery-modal">
      <div class="v179-delivery-summary"><div><small>ENTREGAR TAREA</small><strong>${escHtml(task.title || "Tarea")}</strong><span>${escHtml(participantNames(taskId).join(" · ") || memberNameById(task.assigned_to))}</span></div><em class="v171210-rule-chip ${escHtml(requirement)}">${escHtml(DELIVERY_REQUIREMENT_LABELS[requirement])}</em></div>
      <label class="v179-field full"><span>Comentario o explicación</span><textarea id="v179DeliveryComment" rows="4" placeholder="Resume qué realizaste y qué debe revisar el responsable."></textarea><small>${requirement === "none" ? "Puedes entregar sin evidencia adicional." : "Una explicación clara cuenta como evidencia cuando la tarea acepta cualquier tipo."}</small></label>
      ${linksInputMarkup("v179DeliveryLinks", "Enlaces opcionales")}
      ${fileInputMarkup("v179DeliveryFiles", "Archivos opcionales")}
      <label class="v179-field full"><span>Captura de pantalla opcional</span><input id="v179DeliveryScreenshot" type="file" multiple accept="image/jpeg,image/png,image/webp" capture="environment"><small>Puedes tomar o seleccionar una captura; cuenta como archivo.</small><div class="v179-file-preview" data-v179-preview-for="v179DeliveryScreenshot"></div></label>
      <section class="v171210-effort-card full"><div><span>HORAS REALES</span><strong>¿Cuánto tiempo utilizaste?</strong></div><label>Horas<input id="v1711DeliveryActualHours" type="number" min="0" max="10000" step="1" value="0"></label><label>Minutos<input id="v1711DeliveryActualMinutes" type="number" min="0" max="59" value="0"></label><small>Opcional. Se guarda con la trazabilidad de Trabajo 360.</small></section>
      <section class="v171210-delivery-preview full" id="v171210DeliveryPreview"><span>RESUMEN ANTES DE ENVIAR</span><div id="v171210DeliveryPreviewBody"></div></section>
      <label class="v179-confirm full"><input id="v179DeliveryConfirm" type="checkbox"><span>Confirmo que la tarea está lista para revisión.</span></label>
    </div>`;
    const renderPreview = () => {
      const comment = document.getElementById("v179DeliveryComment")?.value?.trim() || "";
      const links = parseLinks(document.getElementById("v179DeliveryLinks")?.value || "");
      const files = [...selectedFiles("v179DeliveryFiles"), ...selectedFiles("v179DeliveryScreenshot")];
      const hours = Number(document.getElementById("v1711DeliveryActualHours")?.value || 0);
      const minutes = Number(document.getElementById("v1711DeliveryActualMinutes")?.value || 0);
      const host = document.getElementById("v171210DeliveryPreviewBody"); if (!host) return;
      host.innerHTML = `<div><b>${comment ? "Con explicación" : "Sin comentario"}</b><small>${comment ? escHtml(comment.slice(0,120)) : "Puedes añadir una explicación breve."}</small></div><div><b>${links.length} enlace${links.length===1?"":"s"}</b><small>${links.length ? "Se adjuntarán a la entrega." : "Ningún enlace seleccionado."}</small></div><div><b>${files.length} archivo${files.length===1?"":"s"}</b><small>${files.length ? "Incluye archivos y capturas." : "Ningún archivo seleccionado."}</small></div><div><b>${hours || minutes ? `${hours} h ${minutes} min` : "Sin horas"}</b><small>${escHtml(DELIVERY_REQUIREMENT_LABELS[requirement])}</small></div>`;
    };
    openPremiumModal({ title: "Entregar tarea", subtitle: `La entrega pasará a revisión de ${responsibleReviewerName(taskId)}`, body, actions: [
      { label: "Cancelar", value: null, className: "ghost" },
      { label: "Confirmar entrega", className: "primary", loadingLabel: "Enviando…", onClick: async () => {
        if (!document.getElementById("v179DeliveryConfirm")?.checked) throw new Error("Confirma que la tarea está lista para revisión.");
        const files = [...selectedFiles("v179DeliveryFiles"), ...selectedFiles("v179DeliveryScreenshot")];
        const links = parseLinks(document.getElementById("v179DeliveryLinks")?.value || "");
        const comment = document.getElementById("v179DeliveryComment")?.value?.trim() || "";
        ...
        notify("Tarea entregada correctamente", `${when} · Esperando revisión de ${responsibleReviewerName(taskId)}.`, "success");
        return true;
      } }
    ] });
    requestAnimationFrame(() => {
      bindFilePreviews(); renderPreview();
      ["v179DeliveryComment","v179DeliveryLinks","v179DeliveryFiles","v179DeliveryScreenshot","v1711DeliveryActualHours","v1711DeliveryActualMinutes"].forEach((id) => {
        const input = document.getElementById(id); if (!input) return;
        input.addEventListener(input.tagName === "TEXTAREA" || input.type === "number" ? "input" : "change", renderPreview);
      });
    });
  }

  function progressModal(taskId) {
    const task = taskById(taskId); if (!task) return;
    const current = (typeof v412StatusKey === "function" ? v412StatusKey(task.status) : task.status) || "pendiente";
    const normalized = current === "en_proceso" ? "en_proceso" : "pendiente";
    const body = `<div class="v179-modal-grid"><label class="v179-field"><span>Nuevo estado</span><select id="v179ProgressStatus"><option value="pendiente" ${normalized === "pendiente" ? "selected" : ""}>Pendiente</option><option value="en_proceso" ${normalized === "en_proceso" ? "selected" : ""}>En ejecución</option></select></label><label class="v179-field full"><span>Enlace de avance opcional</span><input id="v179ProgressEvidence" type="url" value="${escHtml(task.evidence_url || "")}" placeholder="https://..."></label><label class="v179-field full"><span>Comentario</span><textarea id="v179ProgressComment" rows="4" placeholder="Qué cambió y cuál es el siguiente paso. Para entregar usa el botón principal de la tarea."></textarea></label></div>`;
    openPremiumModal({ title: "Actualizar progreso", subtitle: task.title || "Tarea", body, actions: [
      { label: "Cancelar", value: null, className: "ghost" },
  ...
  function enhanceOpenTask(taskId) {
    ...
    const operation = window.INBESTIGA_V1711?.operation?.(taskId);
    const reviewer = operation?.responsible_member_id ? memberNameById(operation.responsible_member_id) : "responsable final";
    const latest = latestSubmission(taskId);
    const waiting = ["en_revision","corregido"].includes(str(task.status).toLowerCase()) && latest
      ? `<section class="v179-detail-section v171210-awaiting-review"><div><span>ENTREGA REGISTRADA</span><h4>${escHtml(formatDate(latest.submitted_at || latest.created_at))}</h4><p>Esperando revisión de ${escHtml(reviewer)}. La entrega permanece bloqueada hasta que sea aprobada o devuelta para corrección.</p></div></section>` : "";
    wrapper.innerHTML = `${deliveryRequirementDetailMarkup(task)}${waiting}${participantMarkup(taskId)}${referenceMarkup("task", taskId, isManager())}${submissionsMarkup(taskId)}${scoreMarkup(taskId)}`;
    description?.insertAdjacentElement("afterend", wrapper);
    const dock = detail.querySelector(".v66-action-dock");
    if (dock) {
      const taskStatus = str(task.status).toLowerCase();
      const correction = ["observado","observada","requiere_correccion","requiere corrección"].includes(taskStatus);
      const deliveryLocked = ["en_revision","corregido","aprobado","publicado","completado","completada"].includes(taskStatus);
      let deliver = dock.querySelector('[data-v66-task-action="deliver"]');
      const legacyCorrect = dock.querySelector('[data-v66-task-action="correct"]');
      if (!deliver && legacyCorrect) { legacyCorrect.dataset.v66TaskAction = "deliver"; deliver = legacyCorrect; }
      else if (deliver && legacyCorrect) legacyCorrect.remove();
      if (deliver) { deliver.textContent = correction ? "Corregir entrega" : "Entregar tarea"; deliver.hidden = deliveryLocked || !isTaskParticipant(taskId); deliver.disabled = deliveryLocked; }
      const attach = dock.querySelector('[data-v66-task-action="attach"]');
      if (attach) { attach.textContent = isManager() ? "Agregar referencia" : "Adjuntar referencia"; if (!isManager()) attach.hidden = true; }
      const validate = dock.querySelector('[data-v66-task-action="validate"]'); if (validate) validate.textContent = "Aprobar tarea";
      const observe = dock.querySelector('[data-v66-task-action="observe"]'); if (observe) observe.textContent = "Solicitar corrección";
      if (isTaskParticipant(taskId) && !deliver && !dock.querySelector("[data-v179-deliver]") && !deliveryLocked) dock.insertAdjacentHTML("beforeend", `<button type="button" class="primary" data-v179-deliver="${escHtml(taskId)}">${correction ? "Corregir entrega" : "Entregar tarea"}</button>`);
    }
  ...
  async function init() {
    ...
    const passiveRefresh = () => {
      ...
      refreshWorkspace({ render: false }).then(() => { mergeWorkspaceIntoState(); decorateTaskCards(); injectHomeDashboard(); injectWork360Dashboard(); }).catch(() => {});
    };
    ...
    window.INBESTIGA_V179 = { version: "v17.9.2", refresh: refreshWorkspace, workspace: () => workspace, openAttachment, deliveryModal, entityAttachmentModal, checkCloud: () => checkCloud(true), deliveryRequirement: (taskId) => deliveryRequirementFrom(taskById(taskId)), diagnostics: () => ({ cloudAvailable, cloudChecked, lastCheckedAt: cloudLastCheckedAt, lastError: rpcErrorText(cloudLastError) }) };
  }
