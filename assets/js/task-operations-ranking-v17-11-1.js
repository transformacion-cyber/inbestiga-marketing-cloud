/* ===== INBESTIGA v17.11.1 · TASK OPERATIONS · SCHEMA-AWARE RPC HOTFIX (v17.12.5) ===== */
(() => {
  "use strict";

  if (window.INBESTIGA_V1711) return;

  const VERSION = "v17.11";
  ...
  async function init() {
    ...
    setInterval(() => { if (document.visibilityState === "visible") refreshDashboard({ render:false }).then(() => { injectRankingWidgets(); applyCatalogFilters(); }); }, 45000);
  }

  window.INBESTIGA_V1711 = {
    version: "v17.11.1", diagnostics: () => ({ cloudAvailable, lastCheckedAt: cloudLastCheckedAt, lastError: rpcErrorText(cloudLastError) }),
