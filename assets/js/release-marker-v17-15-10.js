/* INBESTIGA Marketing Cloud v17.15.10 · SAKURA CONVERSATION CONTEXT SANITIZATION HOTFIX */
(()=>{
  "use strict";
  const VERSION="v17.15.10",NAME="SAKURA CONVERSATION CONTEXT SANITIZATION HOTFIX",MODULE="release-marker-v17-15-10";
  ...
  const merge=(incoming={})=>{
    ...
    const modules=[...new Set([...(Array.isArray(state.modules)?state.modules:[]),...(Array.isArray(value.modules)?value.modules:[]),MODULE])];
    state={...state,...value,version:VERSION,name:NAME,modules};align();return state;
  };
  merge(state);
  try{
    const descriptor=Object.getOwnPropertyDescriptor(window,"INBESTIGA_BUILD");
    if(!descriptor||descriptor.configurable)Object.defineProperty(window,"INBESTIGA_BUILD",{configurable:false,enumerable:true,get(){return{...state,modules:[...(state.modules||[])]}},set(value){merge(value)}});
  ...
  try{window.INBESTIGA_QUALITY_CORE?.register?.(MODULE,{version:VERSION,mode:"sakura-conversation-context-sanitization",polling:false,realtimeChannels:0,mutationObservers:0,dynamicCode:false,backendChanges:false})}catch{}
})();
