/* INBESTIGA Marketing Cloud v17.15.1 · SAKURA Academy Interaction & Self-Improvement permanent presence */
(() => {
  "use strict";
  if (window.INBESTIGA_SAKURA_LOADER?.version === "v17.15.1") return;
  const VERSION = "v17.15.1";
  const MODULE = "sakura-presence-loader-v17-15-1";
  const runtime = window.INBESTIGA_PUBLIC_RUNTIME_CONFIG || {};
  const flags = Object.assign({enabled:true,chat:true,actions:true,learning:true,voice:true,reports:true,visualDirector:true,web:false,workspaceLayout:true,academy:true,bridgeUrl:"http://127.0.0.1:8765"}, runtime.sakura || {});
  let loading = null;
  ...
  function registerBuild(){try{window.INBESTIGA_QUALITY_CORE?.register?.(MODULE,{version:VERSION,mode:"permanent-lazy-academy-self-improvement",polling:false,realtimeChannels:0})}catch(_){}const build=window.INBESTIGA_BUILD||{};window.INBESTIGA_BUILD={...build,version:VERSION,name:"SAKURA ACADEMY INTERACTION & SUPERVISED SELF-IMPROVEMENT HOTFIX",modules:[...new Set([...(Array.isArray(build.modules)?build.modules:[]),MODULE]) ]};document.documentElement.dataset.inbestigaBuild=VERSION}
