/* INBESTIGA Marketing Cloud v17.15.1 · SAKURA Academy Interaction & Supervised Self-Improvement Hotfix */
(() => {
  "use strict";
  if (window.INBESTIGA_SAKURA_ACADEMY?.version === "v17.15.1") return;

  const VERSION = "v17.15.1";
  const MODULE = "sakura-academy-v17-15-1";
  ...
  let dbPromise = null;
  let mounted = false;
  let state = { activePane:"library", pendingFiles:[], selectedSubject:"all", query:"", cards:[], documents:[], attempts:[], settings:loadSettings() };
  ...
  async function refreshData(){
    const [cards,documents,attempts]=await Promise.all([getAll(CARD_STORE),getAll(DOC_STORE),getAll(ATTEMPT_STORE)]);
    state.cards=cards.filter(card=>card.ownerId===ownerId());state.documents=documents.filter(doc=>doc.ownerId===ownerId());state.attempts=attempts.filter(attempt=>attempt.ownerId===ownerId());
    if(state.settings.autoMirror)mirrorApprovedCards(state.cards);
  ...
  function renderAcademy(){
    ...
    host.innerHTML=`<section class="ska-shell" data-ska-active-pane="${esc(state.activePane)}">
      ...
      <nav class="ska-tabs">${[["library","Biblioteca"],["import","Subir archivos"],["calculator","Calcular"],["practice","Practicar"],["improve","Mejora de SAKURA"],["settings","Control"]].map(([id,label])=>`<button type="button" data-ska-pane="${id}" class="${state.activePane===id?"active":""}">${label}</button>`).join("")}</nav>
      <div class="ska-content">${paneMarkup()}</div>
    </section>`;
    bindAcademy(host);return true;
  }

  function paneMarkup(){switch(state.activePane){case"import":return importPane();case"calculator":return calculatorPane();case"practice":return practicePane();case"improve":return improvementPane();case"settings":return settingsPane();default:return libraryPane()}}
  ...
  function improvementPane(){
    const stats=libraryStats(),attempts=rows(state.attempts),correct=attempts.filter(item=>item.ok).length,accuracy=attempts.length?Math.round(correct*100/attempts.length):0;
    let mirrored=0;try{mirrored=rows(JSON.parse(localStorage.getItem(FALLBACK_KEY)||"[]")).filter(entry=>String(entry.id||"").startsWith("academy:")).length}catch{}
    return `<div class="ska-import-grid"><section class="ska-panel"><h4>Conexión real con SAKURA</h4><p>Las fichas aprobadas se consultan antes de responder y también se envían como conocimiento al modelo local cuando Ollama está activo.</p><div class="ska-hero-stats"><b>${stats.formulas}<small>fórmulas activas</small></b><b>${mirrored}<small>fichas sincronizadas</small></b><b>${accuracy}%<small>precisión de práctica</small></b></div><div class="ska-field"><label>Pregunta de prueba</label><input id="skaTestQuestion" value="Calcula la fuerza con masa 5 y aceleración 3" placeholder="Pregunta algo usando una fórmula aprobada"></div><div class="ska-actions"><button type="button" id="skaTestAcademy" class="sk-primary">Probar en SAKURA</button><button type="button" id="skaRunDiagnostics">Comprobar conexión</button></div><div id="skaSelfImproveResult" class="ska-result">La prueba usa exactamente el mismo motor académico consultado por el chat.</div></section><section class="ska-panel"><h4>Cómo se mejora SAKURA</h4><ul><li>Aprende solo de fichas revisadas y aprobadas.</li><li>Practica fórmulas con un motor determinista, no con respuestas inventadas.</li><li>Guarda tus correcciones del chat como aprendizaje personal.</li><li>Prioriza fuentes, condiciones de uso y unidades.</li><li>No modifica su código, permisos ni personalidad por sí sola.</li></ul><p class="ska-note">“Mejorarse a sí misma” significa recuperar mejor conocimiento, cometer menos errores y aprender de correcciones supervisadas; no reprogramarse sin control.</p></section></div>`;
  }
  function diagnostics(){
    const stats=libraryStats();
    let mirrored=0;try{mirrored=rows(JSON.parse(localStorage.getItem(FALLBACK_KEY)||"[]")).filter(entry=>String(entry.id||"").startsWith("academy:")).length}catch{}
    return {ok:typeof window.INBESTIGA_SAKURA_ACADEMY?.answer==="function"&&stats.approved>0,version:VERSION,approved:stats.approved,formulas:stats.formulas,mirrored,autoMirror:!!state.settings.autoMirror,chatConnected:!!window.INBESTIGA_SAKURA_NATIVE};
  }
  ...
  function bindAcademy(host){
    host.querySelectorAll(".ska-tabs button[data-ska-pane]").forEach(button=>button.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();state.activePane=button.dataset.skaPane;renderAcademy()}));
    ...
    host.querySelector("#skaTestAcademy")?.addEventListener("click",async()=>{const out=host.querySelector("#skaSelfImproveResult"),question=String(host.querySelector("#skaTestQuestion")?.value||"").trim();if(!question){out.textContent="Escribe una pregunta de prueba.";return}out.textContent="Consultando la Academia…";try{const result=await answer(question);out.innerHTML=result?.handled?`<strong>SAKURA utilizó su Academia.</strong><span>${esc(result.text)}</span>`:`<strong>No encontré una coincidencia suficiente.</strong><span>Prueba mencionando la fórmula y sus valores.</span>`}catch(error){out.textContent=error.message}});
    host.querySelector("#skaRunDiagnostics")?.addEventListener("click",()=>{const out=host.querySelector("#skaSelfImproveResult"),result=diagnostics();out.innerHTML=`<strong>${result.ok?"Conexión académica activa":"Conexión incompleta"}</strong><span>Versión ${esc(result.version)} · ${result.formulas} fórmulas · ${result.approved} fichas aprobadas · ${result.mirrored} sincronizadas · chat ${result.chatConnected?"detectado":"pendiente de abrir"}.</span>`});
  ...
  function onLearningClick(event){if(!event.target.closest?.('[data-sk-tab="learning"]'))return;requestAnimationFrame(()=>{ensureMount();setTimeout(ensureMount,60);setTimeout(ensureMount,260)})}

  function registerBuild(){
    try{window.INBESTIGA_QUALITY_CORE?.register?.(MODULE,{version:VERSION,mode:"local-academy-interaction-supervised-learning",polling:false,realtimeChannels:0,mutations:false,dynamicCode:false})}catch{}
    const build=window.INBESTIGA_BUILD||{};window.INBESTIGA_BUILD={...build,version:VERSION,name:"SAKURA ACADEMY INTERACTION & SUPERVISED SELF-IMPROVEMENT HOTFIX",modules:[...new Set([...(Array.isArray(build.modules)?build.modules:[]),MODULE])]};root.dataset.inbestigaBuild=VERSION;
  }
  function health(){const stats=libraryStats();return{status:"ok",value:"SAKURA Academy v17.15.1",detail:`${stats.formulas} fórmulas, ${stats.approved} fichas aprobadas, botones operativos, conexión con chat y mejora supervisada; sin SQL, polling, Realtime, eval ni Function dinámica.`}}
  ...
  const api={version:VERSION,init,search,answer,calculate,convertUnit,molarMass,parseVariables,seedCore,refresh:refreshData,health,diagnostics,mount:ensureMount,state:()=>({...state,cards:[...state.cards],documents:[...state.documents],attempts:[...state.attempts]}),importFiles,approveDocument};
